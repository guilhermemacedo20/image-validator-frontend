    ...
    >
      <div className=" flex flex-col gap-3">
        ...
        <div className="text-xs flex flex-col gap-1">
          <span className={getColor(validations.length)}>
            {validations.length ? "✔" : "•"} Mínimo 8 caracteres
          </span>
          <span className={getColor(validations.upper)}>
            {validations.upper ? "✔" : "•"} Letra maiúscula
          </span>
          <span className={getColor(validations.lower)}>
            {validations.lower ? "✔" : "•"} Letra minúscula
          </span>
          <span className={getColor(validations.number)}>
            {validations.number ? "✔" : "•"} Número
          </span>
          <span className={getColor(validations.special)}>
        ...
        {confirmPassword && (
          <span
            className={`text-xs ${isMatch ? "text-green-600" : "text-red-500"}`}
          >
        ...
        <button
          ...
          className="w-full py-3 font-semibold text-white rounded-lg bg-gradient-to-r from-blue-700 to-blue-500 hover:from-blue-600 hover:to-blue-400 active:scale-[0.98] transition-all shadow-lg"
        >
          Criar conta
        </button>
